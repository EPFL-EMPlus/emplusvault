...
async def create(media: Media, supabase: Client = Depends(authenticate)):
    media_data = create_media(media)
    return media_data


@media_router.get("/media/", response_model=List[Media])
async def get_medias(supabase: Client = Depends(authenticate)):
    return read_media()


@media_router.get("/media/{media_id}", response_model=Media)
async def get_media(media_id: int, supabase: Client = Depends(authenticate)):
