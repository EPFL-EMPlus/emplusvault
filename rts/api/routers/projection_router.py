...
async def create(projection: Projection, supabase: Client = Depends(get_current_active_user)):
    create_projection(projection)
    return {"status": "Projection created"}


@projection_router.get("/projections/")
async def read_projections(supabase: Client = Depends(get_current_active_user)):
    return get_all_projections()


@projection_router.get("/projections/{projection_id}")
async def read_projection(projection_id: int, supabase: Client = Depends(get_current_active_user)):
...
async def read_projection_coordinates(projection_id: int, supabase: Client = Depends(get_current_active_user)):
    return get_projection_coordinates(projection_id)


@projection_router.get("/projections/{projection_id}/atlases")
async def read_projection_atlases(projection_id: int, supabase: Client = Depends(get_current_active_user)):
    return get_projection_atlases(projection_id)


@projection_router.get("/projections/{projection_id}/coordinates/{atlas_id}")
async def read_projection_coordinates_by_atlas(projection_id: int, atlas_id: int, supabase: Client = Depends(get_current_active_user)):
    return get_projection_coordinates_by_atlas(projection_id, atlas_id)


# @projection_router.put("/projections/{projection_id}")
# async def update(projection_id: int, projection: Projection, supabase: Client = Depends(authenticate)):
...
# async def delete(projection_id: int, supabase: Client = Depends(authenticate)):
#     delete_projection(projection_id)
#     return {"status": "Projection deleted"}


@projection_router.get("/coordinates/{projection_id}")
async def get_coordinates(projection_id: int, supabase: Client = Depends(get_current_active_user)):
    return get_projection_coordinates(projection_id)
