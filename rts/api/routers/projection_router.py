...
async def create(projection: Projection, supabase: Client = Depends(authenticate)):
    create_projection(projection)
    return {"status": "Projection created"}


@projection_router.get("/projections/")
async def read_projections(supabase: Client = Depends(authenticate)):
    return get_all_projections()


@projection_router.get("/projections/{projection_id}")
async def read_projection(projection_id: int, supabase: Client = Depends(authenticate)):
...
async def read_projection_coordinates(projection_id: int, supabase: Client = Depends(authenticate)):
    return get_projection_coordinates(projection_id)


@projection_router.get("/projections/{projection_id}/atlases")
async def read_projection_atlases(projection_id: int, supabase: Client = Depends(authenticate)):
    return get_projection_atlases(projection_id)


@projection_router.get("/projections/{projection_id}/coordinates/{atlas_id}")
async def read_projection_coordinates_by_atlas(projection_id: int, atlas_id: int, supabase: Client = Depends(authenticate)):
...
async def get_coordinates(projection_id: int, supabase: Client = Depends(authenticate)):
    return get_projection_coordinates(projection_id)
