...
#     bytes_read = 0
#     with config[f"fs.{media['media_type']}"].open_input_file(media['media_path']) as stream:
#         stream.seek(start)
...
#     media = await get_keyscape_summary(req, media_id)
#     ok = check_file_exists(config[f"fs.{media['media_type']}"], media['media_path'])
#     if not ok:
#         raise HTTPException(status_code=404, detail=f"Media not found {media_id}")

#     total_size = int(media['media_size'])
...
#     content_type = media['media_type'] + '/' + media['media_path'].split('.')[-1]
