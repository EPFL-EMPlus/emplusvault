...
@feature_router.get("/features/projection/{projection_id}")
...
async def get_media_by_library(
    feature_type: str,
    page_size: int = 20,
    ...
    try:
        resp = get_features_by_type_paginated(
            ...
            "*",
            page_size,
            last_seen_feature_id,
            short_clips_only=short_clips_only,
            long_clips_only=long_clips_only,
        )
    except Exception as e:
        raise HTTPException(status_code=401, detail="Not allowed")
    return resp
...
async def get_media_info_by_library(
    feature_type: str,
    page_size: int = 20,
    last_seen_feature_id: int = -1,
    short_clips_only: bool = False,
...
async def get_similar_features(feature_id: str, k_neighbors: int, current_user: User = Depends(get_current_active_user)):
    try:
        ...
        if feature is None:
            ...
            raise HTTPException(status_code=404, detail="Feature not found")
        print("Params: ", feature.feature_id,
              feature.feature_type, 33, k_neighbors)
        resp = get_nearest_neighbors_by_feature(
            feature.feature_id, feature.feature_type, 33, k=k_neighbors)
        return resp
